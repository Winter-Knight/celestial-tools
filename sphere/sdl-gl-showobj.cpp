...
const char * LoadModel(Mesh * model)
{
	...
	static Torus torus1;
	torus1.InitUV(5, 3, 50, 50);
	torus1 = torus1.Double();
	torus1 = torus1.Double();
	torus1.Triangulate();

	model->vertices = torus1.vertices.data();
	model->numVertices = torus1.vertices.size();

	for (i = 0; i < torus1.faces.size(); i++) {
		glm::uvec3 face = torus1.faces[i];
		for (j = 0; j < 3; j++)
...
/*	unsigned int i, j;
	for (i = 0; i < mesh.numVertices; i++)
		printf("vertex[%d]: [%5.2f][%5.2f][%5.2f]\n", i, mesh.vertices[i].x, mesh.vertices[i].y, mesh.vertices[i].z);
	
	for (i = 0; i < mesh.numIndices / 3; i++)
		printf("triangle[%d]: %d %d %d\n", i, mesh.indices[i * 3 + 0], mesh.indices[i * 3 + 1], mesh.indices[i * 3 + 2]);
*/
	...
	SetUniformf("totalVertices", mesh.numVertices);

	// MVPHandler+
	mvphandler.model = glm::rotate(glm::mat4(1.0f), glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
	mvphandler.pos = glm::vec3(0.0f, 0.0f, 20.0f);
